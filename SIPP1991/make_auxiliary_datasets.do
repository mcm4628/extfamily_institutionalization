...
forvalues an = 1/`maxan' {
    gen for_concat_address`an' = SHHADID if (anum == `an')
}
...
keep `i_vars' `j_vars' EEDUC TAGE pp_pnum SHHADID
...
* ego's record
rename pp_pnum pdemo_epppnum
rename TAGE page /* page for "parent age" */

save "$tempdir/person_pdemo", $replace

* create a dataset of household reference persons.
do "$childhh_base_code/SIPP1991/make_aux_refperson"

/* doesn't work because we don't have partner indicators in this panel
