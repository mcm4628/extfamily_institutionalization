...
***************************************************************************
do "$childhh_base_code/SIPP2001/project_macros" /* this do-file contains macros of wave, age, month, relationships */
...
if ("$sipp2001_code" == "") {
    display as error "sipp2001_code macro not set."
    display as error "This macro tells us where to find the project code."
    exit
}

if ("$sipp2001_logs" == "") {
    display as error "sipp2001_logs macro not set."
    display as error "This macro tells us where to put the log files."
    exit
}

confirmdir "$sipp2001_logs"
if `r(confirmdir)' {
    display as error "The sipp2001_logs macro specifies a directory that does not exist:  $sipp2001_logs"
    exit
}

confirmdir "$sipp2001_code"
if `r(confirmdir)' {
    display as error "The sipp2001_code macro specifies a directory that does not exist:  $sipp2001_code"
    exit
}
...
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" extract_and_format

** Combines all the waves into a long file where every person-wave is a record. 
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" merge_waves  
...
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" make_auxiliary_datasets 

** Generates a wide dataset by person (includes static demographic variables). 
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" convert_to_wide 
...
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" normalize_ages 

** Computes biderectional base relationships (mom, dad, child, spouse) 
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" compute_base_relationships 

** Identifies additional relationships transitively
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" compute_secondary_relationships 
...
** reported in a wave
*do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" unify_relationships_across_waves 
...
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" create_comp_change 

** Creates addr_change and hh_change. Converts data file to long. Core file: hh_change.dta
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" create_hh_change 
...
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" create_changer_rels 

** Merges relationship of changers to ego back to comp_change
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" create_HHchangeWithRelationships 
...
do "$childhh_base_code/do_and_log" "$sipp2001_code" "$sipp2001_logs" create_HHComp_asis
