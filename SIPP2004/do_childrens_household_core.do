...
if ("$sipp2004_code" == "") {
    display as error "sipp2004_code macro not set."
    display as error "This macro tells us where to find the project code."
    exit
}

if ("$sipp2004_logs" == "") {
    display as error "sipp2004_logs macro not set."
    display as error "This macro tells us where to put the log files."
    exit
}

confirmdir "$sipp2004_logs"
if `r(confirmdir)' {
    display as error "The sipp2004_logs macro specifies a directory that does not exist:  $sipp2004_logs"
    exit
}

confirmdir "$sipp2004_code"
if `r(confirmdir)' {
    display as error "The sipp2004_code macro specifies a directory that does not exist:  $sipp2004_code"
    exit
}
...
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" merge_waves  
...
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" make_auxiliary_datasets 

** Generates a wide dataset by person (includes static demographic variables). 
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" convert_to_wide 
...
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" normalize_ages 

** Computes biderectional base relationships (mom, dad, child, spouse) 
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" compute_base_relationships 

** Identifies additional relationships transitively
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" compute_secondary_relationships 
...
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" create_comp_change 

** Creates addr_change and hh_change. Converts data file to long. Core file: hh_change.dta
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" create_hh_change 
...
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" create_changer_rels 

** Merges relationship of changers to ego back to comp_change
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" create_HHchangeWithRelationships 
...
do "$childhh_base_code/do_and_log" "$sipp2004_code" "$sipp2004_logs" create_HHComp_asis
