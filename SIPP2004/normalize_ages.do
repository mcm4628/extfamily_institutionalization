...
forvalues w=$first_wave/$final_wave {
  gen in`w'=0
  replace in`w'=1 if !missing(ERRP`w')
  }
  
forvalues w=$first_wave/$penultimate_wave {
  local x=`w'+1
  gen innext`w'=0
...
check fill TAGE* THTOTINC* TFTOTINC* dad_birthplace* dad_yrmigartion* biodad_birthplace* ///
biodad_yrmigration* biomom_birthplace* biomom_yrmigration* mom_birthplace* mom_yrmigartion* educ* dropout* EHHNUMPP*


forvalues wave=$first_wave/$penultimate_wave{
 local x=`wave'+1
 gen dropoutnw`wave'=dropout`x'
...
save "$SIPP04keep/demo_wide.dta", $replace

reshape long adj_age EMS ERRP WPFINWGT EORIGIN EBORNUS ETYPMOM ETYPDAD mom_educ dad_educ mom_immigrant dad_immigrant mom_age biomom_age biomom_educ dad_age biodad_age innext ref_person ref_person_sex ref_person_educ TAGE THTOTINC TFTOTINC dad_birthplace dad_yrmigartion biodad_birthplace biodad_yrmigration biomom_birthplace biomom_yrmigration mom_birthplace mom_yrmigartion educ dropout dropoutnw everdropout EHHNUMPP, i(SSUID EPPPNUM) j(SWAVE)
...
save "$SIPP04keep/demo_long_all", $replace

drop if missing(ERRP)

save "$SIPP04keep/demo_long_interviews", $replace
