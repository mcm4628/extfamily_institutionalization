...
** Function: Reshape data from wide to long. 
...
*  Note: Retain the income variable (it's the same for all records in a HH in any given wave, so we just take the value in the first record).
...
** Output: the household-by-wave data set: hh_wave_characteristics.
save "$tempdir/hh_wave_characteristics", $replace


/******************************** Collapse to one record per household ************************************************/

** Function: Create variables indicating median number of adults and children in households.  
*            Create a varaible indicating a count of number of waves of appearance. (It's OK to count occurrences of num_adults or num_children because neither is ever missing).
...
** Output: save the per household results data set: hh_characteristics.
