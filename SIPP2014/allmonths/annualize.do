...
use "$SIPP14keep/demo_long_all_am.dta", clear

gen year=floor((panelmonth)/12)+1

collapse (firstnm) adj_age par_ed_first my_sex my_race my_racealt WPFINWGT, by(SSUID PNUM year)
...
save "$tempdir/annualized_demo14.dta", replace
...
collapse (sum) comp_change addr_change hh_change (count) nobs=hh_change, by(SSUID PNUM year)
...
merge 1:1 SSUID PNUM panelmonth using "$tempdir/annualized_demo14.dta"
...
save "$SIPP14keep/annualized_change14.dta", replace
